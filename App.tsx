...
import { calculateWorkingDays } from './utils/holidays.ts';
...
const App: React.FC = () => {
  ...
  const fetchAndSetData = useCallback(async () => {
    // Keep previous error for a moment if a background refresh fails
    // setError(null); 
    try {
  ...
  const attendanceStats: AttendanceStats | null = useMemo(() => {
    ...
    const stats = calculateAttendanceStats(rawWeeklyAttendance, currentUserEmail, allDoers, startDate, endDate);
    const defaultStats = { 
            ...
            <main>
              {isLoading ? (
                ...
                <div className="text-center py-10 text-red-600 bg-red-50 rounded-lg shadow-md">
                  ...
                  {error.includes("429") && <p className="mt-2 text-sm text-slate-600">The app is making too many requests to Google Sheets. Please wait for the next automatic refresh.</p>}
                </div>
              ) : (
                <TaskTable
                  tasks={displayTasks}
